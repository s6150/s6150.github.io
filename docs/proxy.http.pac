...
function FindProxyForURL(url, host) {
  ...
  for (let i = 0; i < localTldStorage.length; i++) {
    if (dnsDomainIs(host, localTldStorage[i])) {
      return 'DIRECT';
    }
  }

  for (let i = 0; i < directTldStorage.length; i++) {
    if (dnsDomainIs(host, directTldStorage[i])) {
      return 'DIRECT';
    }
  }

  for (let i = 0; i < directDomainStorage.length; i++) {
    if (dnsDomainIs(host, directDomainStorage[i])) {
      return 'DIRECT';
    }
  }

  for (let i = 0; i < directUrlStorage.length; i++) {
    if (shExpMatch(host, directUrlStorage[i])) {
      return 'DIRECT';
    }
  }

  for (let i = 0; i < directIpStorage.length; i++) {
    if (isInNet(ip, directIpStorage[i], '255.255.255.255')) {
      return 'DIRECT';
    }
  }
  ...
  if ((!proxyDomain || proxyDomainStorage.length === 0) || (!proxyUrl || proxyUrlStorage.length === 0)) {
    ...
    for (let i = 0; i < proxyDomainStorage.length; i++) {
      if (dnsDomainIs(host, proxyDomainStorage[i])) {
        return proxyServer;
      }
    }

    for (let i = 0; i < proxyUrlStorage.length; i++) {
      if (shExpMatch(host, proxyUrlStorage[i])) {
        return proxyServer;
      }
    }
  }
